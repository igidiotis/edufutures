...
export const useResearchStore = create<ResearchState>()(
  persist(
    (set, get) => ({
      // Consent state - default is false (not consented)
      hasConsent: false,
    ...
    {
      name: 'edufutures-storage', // name for the storage key
      partialize: (state) => ({ hasConsent: state.hasConsent }), // only persist the consent state
    }
  )
); 